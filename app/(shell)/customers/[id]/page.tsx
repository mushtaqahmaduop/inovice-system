...
export default async function CustomerLedgerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="w-full px-5 py-6 md:px-8">
      ...
      {/* Invoices */}
      <p className="mono mb-2 text-[9px] tracking-[0.16em] text-text-tertiary uppercase">
        Invoices
      </p>

      {/* Below sm the invoice table becomes stacked cards keyed by number. */}
      <div className="mb-6 border border-border bg-surface sm:hidden">
        {rows.map((r) => (
          <Link
            key={r.id}
            href={r.status === "draft" ? `/invoices/${r.id}/edit` : `/invoices/${r.id}`}
            className="block border-b border-border px-3 py-2.5 last:border-b-0 hover:bg-accent/50"
          >
            ...
            <div className="mt-1 flex items-center justify-between gap-2">
              <span className="mono text-[11px] text-text-tertiary">{r.issue_date ?? "—"}</span>
              <span className="mono text-[9px] tracking-[0.1em] uppercase">
                ...
                {r.status === "issued" ? (
                  <span
                    className={`ml-2 ${r.payment_status === "paid" ? "text-success" : "text-text-tertiary"}`}
                  >
                    {r.payment_status ?? "—"}
                  </span>
                ) : null}
        ...
        {rows.length === 0 ? (
          <p className="px-3 py-6 text-center text-[12px] text-text-tertiary">No invoices yet.</p>
        ) : null}
      </div>

      <div className="mb-6 hidden overflow-x-auto border border-border bg-surface sm:block">
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-border">
              {["Number", "Issued", "Total", "Paid", "Status", "Payment"].map((h) => (
                <th
                  key={h}
                  className="mono px-3 py-2 text-[9px] tracking-[0.14em] text-text-tertiary uppercase"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-border last:border-b-0 hover:bg-accent/50">
                ...
                <td className="mono px-3 py-2 text-[11.5px] text-text-tertiary">
                  {r.issue_date ?? "—"}
                </td>
                ...
                <td className="mono px-3 py-2 text-[9px] tracking-[0.1em] text-text-tertiary uppercase">
                  {r.status === "issued" ? "· sealed ·" : r.status}
                </td>
                <td className="mono px-3 py-2 text-[9px] tracking-[0.1em] uppercase">
                  <span
                    className={r.payment_status === "paid" ? "text-success" : "text-text-tertiary"}
                  >
                    {r.payment_status ?? "—"}
                  </span>
                </td>
            ...
            {rows.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-3 py-6 text-center text-[12px] text-text-tertiary">
                  No invoices yet.
                </td>
              </tr>
            ) : null}
      ...
      <div className="divide-y divide-border border border-border bg-surface">
        {(payments ?? []).map((p) => (
          <div key={p.id} className="flex flex-wrap items-center gap-3 px-3 py-2">
            ...
            <span
              className={`mono w-28 text-right text-[12px] ${p.amount < 0 ? "text-danger" : "text-foreground"}`}
            >
        ...
        {(payments ?? []).length === 0 ? (
          <p className="px-3 py-6 text-center text-[12px] text-text-tertiary">No payments yet.</p>
        ) : null}
      </div>
    </div>
  );
...
function Stat({ label, value, warn }: { label: string; value: string; warn?: boolean }) {
  return (
    <div className="border border-border bg-surface p-3">
      <p className="mono mb-1 text-[9px] tracking-[0.14em] text-text-tertiary uppercase">{label}</p>
      <p className={`mono text-[15px] font-medium ${warn ? "text-danger" : "text-foreground"}`}>
        {value}
      </p>
    </div>
  );
